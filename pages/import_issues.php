...
?>
    ...
    <div class="col-md-12 col-xs-12">
        ...
        <div class="alert <?php echo $t_alert_style; ?>">
            ...
            $t_message .= sprintf( plugin_lang_get( 'result_update_success_ct'  ), $t_success_count['update']) . '<br />';
            $t_message .= sprintf( plugin_lang_get( 'result_import_success_ct'  ), $t_success_count['new']) . '<br />';
            $t_message .= sprintf( plugin_lang_get( 'result_nothing_success_ct'  ), $t_success_count['nothing']) . '<br />';

            if( $t_failure_count ) {
                $t_message .= '<b>'.sprintf( plugin_lang_get( 'result_failure_ct' ), $t_failure_count) . ' :</b><br />' . $t_error_messages . '<br/>';
            }

            if ($t_failure_count) {
                html_operation_failure($t_redirect_url, $t_message);
            } else {
